...
#if extra_techniques
// solution with extra techniques(Double Buffering)
static void *all_techniques_worker_func(void *args) {
    ...
    for (int row = 0; row < m; row++) {
        ...
        // double buffering
            __m256i raw_w_group[2], raw_w_next_group[2];
            __m256i raw_w_0, raw_w_128, raw_w_0_next, raw_w_128_next;
            int load_idx = 0, compute_idx = 1;
            const __m256i lowMask = _mm256_set1_epi8(0xF);
            const __m256i zero_point = _mm256_set1_epi8(8);
        // first-time loading
            raw_w_group[load_idx] = _mm256_loadu_si256(w_start);
            raw_w_next_group[load_idx] = _mm256_loadu_si256(w_start + 1);
            w_start += 2;
        // main loop
            for (int q = 0; q < num_block - 4; q += 4) {
            // next-iteration loading & current iteration computing
                load_idx = (q & 7) == 0 ? 1 : 0;
                compute_idx = (q & 7) == 0 ? 0 : 1;

                raw_w_group[load_idx] = _mm256_loadu_si256(w_start);
                raw_w_next_group[load_idx] = _mm256_loadu_si256(w_start + 1);

                raw_w_0 = _mm256_and_si256(raw_w_group[compute_idx], lowMask);
                raw_w_128 = _mm256_and_si256(_mm256_srli_epi16(raw_w_group[compute_idx], 4), lowMask);
                raw_w_0_next = _mm256_and_si256(raw_w_next_group[compute_idx], lowMask);
                raw_w_128_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next_group[compute_idx], 4), lowMask);

                __m256i w_0 = _mm256_sub_epi8(raw_w_0, zero_point);
                __m256i w_128 = _mm256_sub_epi8(raw_w_128, zero_point);
                __m256i w_0_next = _mm256_sub_epi8(raw_w_0_next, zero_point);
                __m256i w_128_next = _mm256_sub_epi8(raw_w_128_next, zero_point);
        ...
        // last-time computing
            raw_w_0 = _mm256_and_si256(raw_w_group[load_idx], lowMask);
            raw_w_128 = _mm256_and_si256(_mm256_srli_epi16(raw_w_group[load_idx], 4), lowMask);
            raw_w_0_next = _mm256_and_si256(raw_w_next_group[load_idx], lowMask);
            raw_w_128_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next_group[load_idx], 4), lowMask);
...
#else
// original solutions(include loop_unrolling/SIMD/multithreading)
static void *all_techniques_worker_func(void *args) {
    struct w4a8_thread_args *mat_args = (struct w4a8_thread_args *)args;
    const struct matmul_params *params = mat_args->params;
    const struct matrix *A = &params->A, *B = &params->B, *C = &params->C;
    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i raw_w_0 = _mm256_and_si256(raw_w, lowMask);
                __m256i raw_w_128 = _mm256_and_si256(_mm256_srli_epi16(raw_w, 4), lowMask);
                __m256i raw_w_0_next = _mm256_and_si256(raw_w_next, lowMask);
                __m256i raw_w_128_next = _mm256_and_si256(_mm256_srli_epi16(raw_w_next, 4), lowMask);
                ...
                w_0 = _mm256_sub_epi8(raw_w_0, zero_point);
                w_128 = _mm256_sub_epi8(raw_w_128, zero_point);
                w_0_next = _mm256_sub_epi8(raw_w_0_next, zero_point);
                w_128_next = _mm256_sub_epi8(raw_w_128_next, zero_point);
...
namespace matmul {
#if extra_techniques
// solution with extra techniques
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    for(int i = 0; i < num_thread; i++){
        ...
        threads_args[i].end_j = MIN(((i + 1) * n) / num_thread, n);
    ...
    pthread_join(thread_pool[5], NULL);
    pthread_join(thread_pool[6], NULL);
    pthread_join(thread_pool[7], NULL);
};
#else
// original solutions(include loop_unrolling/SIMD/multithreading)
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
